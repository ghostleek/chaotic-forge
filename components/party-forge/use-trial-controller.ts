...
import type {
  KitchenPartyRuntime,
  KitchenPartySnapshot,
} from '../../lib/party-forge/runtimes/kitchen-chaos-v1/adapter.ts';
...
export type TrialView = {
  status: Status;
  snapshot: KitchenPartySnapshot | null;
  message: string;
  countdown: number;
  seconds: number;
  feedback: string;
...
class TrialController {
  ...
  private runtime: KitchenPartyRuntime | null = null;
  ...
  private async load(room: RoomSnapshot) {
    ...
    try {
      ...
      const runtime = await createPartyRuntime(
        room.build.manifest,
        room.round.seed,
      );
  ...
  private observe() {
    ...
    this.update({
      status: 'countdown',
      message: 'The shared round is starting. Click the viewport to aim.',
    });
  }
  private tick() {
    if (
    ...
    try {
      ...
      while (this.capture.frames.length < due) {
        ...
        const last = step.state.events.at(-1);
