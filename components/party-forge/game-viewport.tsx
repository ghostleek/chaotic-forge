  ...
  return <section className={styles.game} ref={region} aria-label="Generated pixel game">
    ...
    <div className={styles.controlRow}>
      <div id="trial-controls" className={styles.controls}>
        ...
        <p>60 seconds. Most points wins. Fewer hits breaks a tie.</p>
        <p className={styles.feedback}>{view.feedback || 'EAT. GROW. ZAP. REPEAT.'}</p>
      </div>
      <div className={styles.dpad} aria-label="Touch direction pad">
        {([['↑', 1, 'Up'], ['←', 4, 'Left'], ['↓', 2, 'Down'], ['→', 8, 'Right']] as const).map(([glyph, bit, label]) => <button key={label} type="button" aria-label={`Steer ${label.toLowerCase()}`} data-dir={label} onPointerDown={e => { e.preventDefault(); e.currentTarget.setPointerCapture(e.pointerId); controller.buttons(bit); }} onPointerUp={() => controller.clearInput()} onPointerCancel={() => controller.clearInput()} onLostPointerCapture={() => controller.clearInput()} onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); controller.buttons(bit); } }} onKeyUp={() => controller.clearInput()}>{glyph}</button>)}
      </div>
    </div>
    <output aria-live="polite">{view.message}</output>
    <p className={styles.note}>Same start. Separate arenas. Your score is confirmed after the round. The timer continues when you leave this tab.</p>
  </section>;
}
