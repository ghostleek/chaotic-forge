...
export function TrialHud({ view }: { view: TrialView }) {
  ...
  return <div className={styles.hud}>
    ...
    <div className={styles.scores}>
      ...
      <span><small>{typeof view.snapshot?.state.lives === 'number' ? 'LIVES' : 'HITS'}</small><strong>{typeof view.snapshot?.state.lives === 'number' ? `${view.snapshot.state.lives}/3` : String(state?.hits ?? 0).padStart(2, '0')}</strong></span>
    </div>
  </div>;
}
