...
export type KitchenRecipe = {
  fps: 'knockback' | 'counter-ricochet';
  zombies: 'pursuers' | 'noise-seekers';
  cooking: 'quick-orders' | 'batch-orders';
  additions: Array<'dinner-bell' | 'hot-potato' | 'zombie-pantry'>;
};

/** Full held button state, not a list of commands. Shoot/interact use edges. */
export type InputFrame = { tick: number; buttons: number; yaw: number; pitch: number };
...
type Counter = { id: string; minX: number; maxX: number; minY: number; maxY: number; height: number; ricochet: boolean };
...
export type KitchenCarry = { kind: 'raw' | 'prepared' | 'dish'; portions: number; acquiredAtTick: number };
export type KitchenZombie = Point & {
  id: number;
  hp: number;
  stunnedUntilTick: number;
  target: 'player' | 'stove' | 'bell';
  targetX: number;
  targetY: number;
  waypoint: Point | null;
};
export type KitchenEvent = {
  type: 'shot' | 'hit' | 'ricochet' | 'zombie_spawned' | 'zombie_repelled' | 'contact' |
    'ingredient_picked' | 'prepared' | 'cook_loaded' | 'cook_started' | 'cook_ready' |
    'dish_collected' | 'order_delivered' | 'dish_spoiled' | 'order_burned' |
    'pantry_dropped' | 'pantry_picked' | 'bell_rang' | 'completed';
  tick: number;
  entityId?: number;
  x?: number;
  y?: number;
...
export type KitchenMetrics = {
  ...
  pantryDrops: number;
  pantryPickups: number;
  contactHits: number;
  spoiledDishes: number;
  timerSpoiledDishes: number;
  burnedOrders: number;
  dinnerBellActivations: number;
  pursuitTicks: number;
  noiseSeekingTicks: number;
  bellAttractionTicks: number;
};
export type KitchenRuntimeState = {
  ...
  nextEntityId: number;
  player: Point & { yaw: number; pitch: number; carry: KitchenCarry | null; slowedUntilTick: number; contactReadyTick: number };
  stove: { portions: number; readyAtTick: number | null; burnAtTick: number | null };
  bell: { x: number; y: number; untilTick: number } | null;
  zombies: KitchenZombie[];
  drops: Array<Point & { id: number; expiresAtTick: number }>;
  metrics: KitchenMetrics;
  /** Only this tick's events; consumers can retain a bounded presentation log. */
  events: KitchenEvent[];
};
export type KitchenSnapshot = KitchenRuntimeState & { remainingTicks: number };

const clamp = (value: number, low: number, high: number) => Math.min(high, Math.max(low, value));
const quantize = (value: number) => Math.round(value * RULES.positionPrecision) / RULES.positionPrecision;
const distance = (a: Point, b: Point) => Math.hypot(a.x - b.x, a.y - b.y);
const portionsFor = (recipe: KitchenRecipe) => recipe.cooking === 'quick-orders' ? RULES.quickOrderPortions : RULES.batchOrderPortions;
const hasAddition = (state: KitchenRuntimeState, addition: KitchenRecipe['additions'][number]) => state.recipe.additions.includes(addition);

function validateRecipe(recipe: KitchenRecipe) {
...
function validateFrame(state: KitchenRuntimeState, frame: InputFrame) {
...
export function createRuntime(recipe: KitchenRecipe, seed: number): KitchenRuntimeState {
...
function cloneState(state: KitchenRuntimeState): KitchenRuntimeState {
  return {
...
export function snapshotRuntime(state: KitchenRuntimeState): KitchenSnapshot {
  return { ...cloneState(state), remainingTicks: RULES.durationTicks - state.tick };
}

export function resetRuntime(state: KitchenRuntimeState): KitchenRuntimeState {
  return createRuntime(state.recipe, state.seed);
}

function emit(state: KitchenRuntimeState, event: Omit<KitchenEvent, 'tick'>) {
  if (state.events.length < RULES.maxEventsPerTick) state.events.push({ ...event, tick: state.tick });
}

function random(state: KitchenRuntimeState) {
  let value = state.rng;
  value ^= value << 13;
  value ^= value >>> 17;
  value ^= value << 5;
...
function canOccupy(point: Point, radius: number) {
...
function move(point: Point, deltaX: number, deltaY: number, radius: number) {
  ...
  for (let step = 0; step < steps; step += 1) {
    ...
    if (canOccupy({ x, y: point.y }, radius)) point.x = x;
    const y = quantize(clamp(point.y + deltaY / steps, radius, WORLD.height - radius));
    if (canOccupy({ x: point.x, y }, radius)) point.y = y;
  }
}

type Ray = { x: number; y: number; z: number; dx: number; dy: number; dz: number };
type SurfaceHit = { distance: number; nx: number; ny: number; nz: number; ricochet: boolean };

/** Slab intersection with a closed box; direction is unit length. */
function rayBox(ray: Ray, low: [number, number, number], high: [number, number, number]): SurfaceHit | null {
  ...
  if (far < RULES.rayEpsilon) return null;
  return { distance: Math.max(0, near), nx: normal[0], ny: normal[1], nz: normal[2], ricochet: false };
}

function nearestSurface(ray: Ray): SurfaceHit {
  let nearest: SurfaceHit = { distance: RULES.shotRange, nx: 0, ny: 0, nz: 0, ricochet: false };
  for (const counter of WORLD.counters) {
    ...
    if (hit && hit.distance < nearest.distance) nearest = { ...hit, ricochet: counter.ricochet };
  }
  ...
  for (const [origin, direction, extent, axis] of [
    ...
    if (Math.abs(direction) < RULES.rayEpsilon) continue;
...
function rayZombie(ray: Ray, zombie: KitchenZombie): number | null {
  ...
  if (a < RULES.rayEpsilon * RULES.rayEpsilon) {
    if (c > 0 || Math.abs(ray.dz) < RULES.rayEpsilon) return null;
  ...
  if (Math.abs(ray.dz) < RULES.rayEpsilon) {
    if (ray.z < 0 || ray.z > RULES.zombieHeight) return null;
  } else {
...
function addPantryDrop(state: KitchenRuntimeState, zombie: KitchenZombie) {
  if (!hasAddition(state, 'zombie-pantry') || state.metrics.pantryPickups + state.drops.length >= RULES.pantryPickupCap || state.drops.length >= RULES.pantryWorldDropCap) return;
...
function shoot(state: KitchenRuntimeState) {
  ...
  for (let bounce = 0; bounce <= RULES.maxShotBounces; bounce += 1) {
    ...
    if (target) {
      ...
      if (target.hp <= 0) state.zombies = state.zombies.filter((zombie) => zombie.id !== target.id);
      return; // Exactly one zombie can be hit by one shot, including a bounce.
    }
    if (state.recipe.fps !== 'counter-ricochet' || !surface.ricochet || surface.distance >= remaining || bounce === RULES.maxShotBounces) return;
...
function spoilCarry(state: KitchenRuntimeState, reason: 'timer' | 'contact') {
  if (state.player.carry?.kind !== 'dish') return;
  state.metrics.failedOrders += 1;
  state.metrics.spoiledDishes += 1;
  if (reason === 'timer') state.metrics.timerSpoiledDishes += 1;
  emit(state, { type: 'dish_spoiled', portions: state.player.carry.portions, reason });
  state.player.carry = null;
}

function interact(state: KitchenRuntimeState) {
...
function blockedPath(from: Point, to: Point, radius: number): Counter | null {
  const length = distance(from, to);
  if (length < RULES.rayEpsilon) return null;
  const ray = { x: from.x, y: from.y, z: 0.5, dx: (to.x - from.x) / length, dy: (to.y - from.y) / length, dz: 0 };
  let nearest: Counter | null = null;
  ...
  for (const counter of WORLD.counters) {
    ...
    if (hit && hit.distance < nearestDistance) { nearest = counter; nearestDistance = hit.distance; }
  }
  return nearest;
}

function zombieDestination(zombie: KitchenZombie, target: Point): Point {
  const obstacle = blockedPath(zombie, target, RULES.zombieRadius);
  if (!obstacle) { zombie.waypoint = null; return target; }
  if (zombie.waypoint && distance(zombie, zombie.waypoint) > RULES.waypointMargin) return zombie.waypoint;
  ...
  options.sort((a, b) => {
    const estimate = (point: Point) => distance(zombie, point) + distance(point, target) + (blockedPath(point, target, RULES.zombieRadius) ? 2 : 0);
...
function updateZombies(state: KitchenRuntimeState) {
  if (state.tick >= state.nextSpawnTick) {
    ...
    if (state.zombies.length < RULES.maxZombies) {
      const zombie: KitchenZombie = { id: state.nextEntityId++, x, y, hp: RULES.zombieHealth, stunnedUntilTick: 0, target: 'player', targetX: state.player.x, targetY: state.player.y, waypoint: null };
      state.zombies.push(zombie);
      emit(state, { type: 'zombie_spawned', entityId: zombie.id, x, y });
    }
  }
  for (const zombie of state.zombies) {
    let target: Point = state.player;
    let kind: KitchenZombie['target'] = 'player';
    ...
    if (kind !== zombie.target) zombie.waypoint = null;
    zombie.target = kind;
    zombie.targetX = target.x;
    zombie.targetY = target.y;
    if (state.tick < zombie.stunnedUntilTick) continue;
    ...
    if (remaining > RULES.rayEpsilon) move(zombie, ((destination.x - zombie.x) / remaining) * Math.min(speed, remaining), ((destination.y - zombie.y) / remaining) * Math.min(speed, remaining), RULES.zombieRadius);
    if (distance(before, zombie) > RULES.rayEpsilon) {
      if (kind === 'stove') state.metrics.noiseSeekingTicks += 1;
      else if (kind === 'bell') state.metrics.bellAttractionTicks += 1;
      else state.metrics.pursuitTicks += 1;
    }
...
export function stepRuntime(state: KitchenRuntimeState, frame: InputFrame): KitchenRuntimeState {
  ...
  if (next.bell && next.tick >= next.bell.untilTick) next.bell = null;
  ...
  if (hasAddition(next, 'hot-potato') && next.player.carry?.kind === 'dish' && next.tick - next.player.carry.acquiredAtTick >= RULES.hotPotatoCarryTicks) spoilCarry(next, 'timer');
  ...
  if (pressed & BUTTONS.shoot && next.tick >= next.nextShotTick) shoot(next);
  if (pressed & BUTTONS.interact) interact(next);
  updateZombies(next);
...
export function replayRuntime(recipe: KitchenRecipe, seed: number, frames: readonly InputFrame[]): KitchenRuntimeState {
  if (!Array.isArray(frames) || frames.length !== RULES.durationTicks) throw new RangeError('A scored trial requires exactly 3600 input frames.');
  let state = createRuntime(recipe, seed);
  for (const frame of frames) state = stepRuntime(state, frame);
  return state;
}
