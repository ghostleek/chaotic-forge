import type { PixelRecipe } from '../pixel-arcade-v1/rules.ts';
...
export type PixelState = {
  ...
  lives: number; gameOver: boolean; points: number; hits: number; kills: number; eaten: number; feedback: string; flashUntil: number;
...
export class PixelRuntime implements PartyRuntime {
  ...
  step(): PixelSnapshot {
    ...
    this.pending = null;
    const s = this.state;
    if (s.gameOver) { this.tick++; return this.snapshot(); }
