...
export async function resolveInstructionRequest(request:ResolveRequest, db:D1Database, roomId:string):Promise<ResolveResult> {
  ...
  const recipeKey=await hashValue({parent:previous?.contentHash??null,cards});
  ...
  if(!stored){
    ...
    if(claimed.meta.changes===0){
      ...
      try{
        const response=await fetch('https://api.openai.com/v1/responses',{method:'POST',redirect:'manual',signal:AbortSignal.timeout(55_000),headers:{Authorization:`Bearer ${key}`,'Content-Type':'application/json'},body:JSON.stringify({model:'gpt-6-astra',reasoning:{effort:'low'},max_output_tokens:3000,store:false,instructions,input:JSON.stringify({cards,previousRules:previous?.pixelRules??null}),text:{format:{type:'json_schema',name:'pixel_recipe',strict:true,schema:z.toJSONSchema(outputSchema)}}})});
        ...
        if(data.status!=='completed') throw new Error('Generation did not finish within the output limit.');
        ...
        if(previous?.pixelRules){
          ...
          if([...protectedFields].some(field=>output.recipe[field]!==prior[field])) throw new Error('The new instructions would replace an earlier rule. Revise the next cards to add something compatible.');
        }
        ...
        await db.prepare("UPDATE party_pixel_jobs SET status='failed',result=? WHERE room_id=? AND recipe_key=? AND status='running'").bind(JSON.stringify({reason}),roomId,recipeKey).run();
        return fail(reason);
      }
    }
  }
