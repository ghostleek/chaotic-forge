...
type Stored = {status:string;result:string|null;createdAt:number};
export const PIXEL_JOB_LEASE_MS = 90_000;
async function loadStoredPixelJob(db:D1Database, roomId:string, recipeKey:string){
  return db.prepare('SELECT status,result,created_at AS createdAt FROM party_pixel_jobs WHERE room_id=? AND recipe_key=?').bind(roomId,recipeKey).first<Stored>();
}
export async function claimPixelJob(db:D1Database, roomId:string, recipeKey:string, now=Date.now(), existing:Stored|null=null){
  let stored=existing ?? await loadStoredPixelJob(db,roomId,recipeKey);
  if(!stored){
    const claimed=await db.prepare("INSERT OR IGNORE INTO party_pixel_jobs(room_id,recipe_key,status,created_at) SELECT ?,?,'running',? WHERE (SELECT COUNT(*) FROM party_pixel_jobs WHERE room_id=?)<3").bind(roomId,recipeKey,now,roomId).run();
    if(claimed.meta.changes===1) return {kind:'new' as const,stored:{status:'running',result:null,createdAt:now}};
    stored=await loadStoredPixelJob(db,roomId,recipeKey);
    if(!stored) return {kind:'room-limit' as const,stored:null};
  }
  if(stored.status==='running' && stored.createdAt<=now-PIXEL_JOB_LEASE_MS){
    const reclaimed=await db.prepare("UPDATE party_pixel_jobs SET created_at=? WHERE room_id=? AND recipe_key=? AND status='running' AND created_at<=?").bind(now,roomId,recipeKey,now-PIXEL_JOB_LEASE_MS).run();
    if(reclaimed.meta.changes===1) return {kind:'recovered' as const,stored:{...stored,createdAt:now}};
    stored=await loadStoredPixelJob(db,roomId,recipeKey);
  }
  return {kind:'existing' as const,stored};
}
export async function resolveInstructionRequest(request:ResolveRequest, db:D1Database, roomId:string):Promise<ResolveResult> {
  ...
  if(!previous){
    const cached=await cachedInstructionDemo(contributions);
    if(cached) return {status:'playable',manifest:cached};
  }
  ...
  let stored=await loadStoredPixelJob(db,roomId,recipeKey);
  ...
  const claim=await claimPixelJob(db,roomId,recipeKey,Date.now(),stored);
  stored=claim.stored;
  if(claim.kind==='room-limit') return fail('This room has used its three generation attempts. Keep the last game, or create a new room.');
  if(claim.kind==='new' || claim.kind==='recovered'){
    ...
    const idempotencyKey=await hashValue({kind:'party-pixel-job',roomId,recipeKey});
    if(claim.kind==='new'){
    ...
    try{
      const response=await fetch('https://api.openai.com/v1/responses',{method:'POST',redirect:'manual',signal:AbortSignal.timeout(55_000),headers:{Authorization:'Bearer '+key,'Content-Type':'application/json','Idempotency-Key':idempotencyKey},body:JSON.stringify({model:'gpt-6-astra',reasoning:{effort:'low'},max_output_tokens:PIXEL_OUTPUT_TOKENS,store:false,instructions,input:JSON.stringify({cards,previousRules:previous?.pixelRules??null}),text:{format:{type:'json_schema',name:'pixel_recipe',strict:true,schema:z.toJSONSchema(outputSchema)}}})});
      ...
      const updated=await db.prepare("UPDATE party_pixel_jobs SET status='complete',result=? WHERE room_id=? AND recipe_key=? AND status='running'").bind(result,roomId,recipeKey).run();
      stored=updated.meta.changes===1 ? {status:'complete',result,createdAt:stored?.createdAt??Date.now()} : await loadStoredPixelJob(db,roomId,recipeKey);
    }catch(error){
      ...
      const updated=await db.prepare("UPDATE party_pixel_jobs SET status='failed',result=? WHERE room_id=? AND recipe_key=? AND status='running'").bind(JSON.stringify({reason,diagnostic}),roomId,recipeKey).run();
      if(updated.meta.changes===0) stored=await loadStoredPixelJob(db,roomId,recipeKey);
