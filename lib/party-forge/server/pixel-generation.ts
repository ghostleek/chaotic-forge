...
import { outputSchema, PIXEL_OUTPUT_TOKENS, PIXEL_GENERATION_POLICY, generationFailureReason, shouldRecoverLegacyOutputFailure } from './pixel-generation-policy.ts';
  ...
  if(!stored){
    ...
    if(claimed.meta.changes===0){
      ...
      try{
        ...
        console.error('pixel_generation_failed',JSON.stringify({roomId,policy:PIXEL_GENERATION_POLICY,...diagnostic}));
        return fail(reason);
      }
    }
  }
