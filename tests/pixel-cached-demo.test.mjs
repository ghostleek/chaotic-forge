...
void test('cached pair works in either order without database or model access',async()=>{
  for(const titles of [['Snake','Space Invaders'],['  SPACE  INVADERS ','snake'],['Space invader','SNAKES'],['snakes','space invader'],['SPACE-INVADER!','Snake'],['Space invader','Snake','SNAKES']]){
    ...
    for(const [index,title] of titles.entries()){
      const field=/^snakes?$/.test(title.trim().toLowerCase())?'growTail':'invaders';
      assert.ok(build.pixelRules.interpretations.find(i=>i.instructionIndex===index).ruleFields.includes(field));
    }
...
void test('audio and rhythm requests are blocked before a generation job is accessed',async()=>{
 for(const instruction of ['Add AUDIO','Sound turns off every 5 seconds','taptap revenge','Tap Tap Revenge','dance dance revlution','Dance Dance Revolution','DDR','Add music','Rhythm lanes']){
   const result=await resolveInstructionRequest({contributions:cards(['Snake',instruction])},{prepare(){throw new Error('Unsupported cards must not access model jobs');}},'blocked-room');
...
void test('new mechanic terms enter normal generation instead of the demo cache',async()=>{
 let lookups=0;
 const db={prepare(){lookups++;return {bind(){return {async first(){return {status:'failed',result:JSON.stringify({reason:'Normal generation path'})};}};}};}};
 for(const instruction of ['Snake with wrapping','Snake with more food','Snake and tap to shoot']){
   const result=await resolveInstructionRequest({contributions:cards([instruction,'Space invader'])},db,'custom-room');
   assert.equal(result.status,'incompatible');
